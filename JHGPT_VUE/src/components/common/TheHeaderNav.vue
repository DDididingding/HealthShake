  ...
  <div id="container">
    <header v-if="!isLoggedin">
      ...
      <router-link
        to="/reviewregist"
        class="nav-link"
        :class="{ 'active-link': $route.path === '/review_regist' }"
        >테스트용리뷰등록</router-link
      >
      <router-link
        to="/login"
        class="nav-link"
      ...
      <router-link
        to="/reviewList"
        class="nav-link"
        :class="{ 'active-link': $route.path === '/reviewList' }"
        >reviewList</router-link
      >
    </header>

    <header v-else-if="isLoggedin && userType === 1">
      <!-- 유저가 로그인 했을 때 -->
      <router-link
        to="/homeUser"
        class="nav-link"
        :class="{ 'active-link': $route.path === '/homeUser' }"
        ><h1>JHGPT</h1></router-link
      >
      <router-link
        :to="{ name: 'UserBuylist', params: { member_code: member_code } }"
        class="nav-link"
        :class="{ 'active-link': $route.name === 'UserBuylist' }"
        >구매 목록</router-link
      >
      <router-link
        :to="{ name: 'userMypage', params: { member_code: member_code } }"
        class="nav-link"
        :class="{ 'active-link': $route.name === 'userMypage' }"
        >userMypage</router-link
      >
    ...
    <header v-else-if="isLoggedin && userType === 2">
      ...
      <router-link
        ...
        class="nav-link"
        :class="{ 'active-link': $route.name === 'trainerMypage' }"
        >trainerMypage</router-link
      >
      <button @click="handleLogout" class="nav-link">Logout</button>
      <!--이 밑으로는 테스트 용-->
      <!-- <nav> -->

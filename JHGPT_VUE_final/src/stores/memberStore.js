  ...
  () => {
      ...
      try {
        ...
        return new Promise((resolve, reject) => {
          if (responseMember !== null) {
            if (
              responseMember.member_password === inputMember.member_password
            ) {
    ...
    const buyTrainer = (user_member_code, trainer_member_code) => {
      return new Promise((resolve, reject) => {
        axios
          .post(
            `http://localhost:9999/api/buytrainer?user_member_code=${user_member_code}&trainer_member_code=${trainer_member_code}`
          )
          .then(() => {
    ...
    return {
      ...
      setLoginMember,
      getMemberList,
      getMemberListPromise,
      getTrainerList,
      getTrainerListPromise,
      getUserList,
      updateTrainerMy,
      updateUserMy,
      trainerSignup,
      selectTrainerPromise,
      trainers,
      trainer,
      userSignup,
      selectTrainer,

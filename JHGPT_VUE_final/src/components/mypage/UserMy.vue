<template>
          ...
          <div class="profile-section">
            ...
            <div class="profile-details" style="display: flex; justify-content: flex-start;">
              ...
              <div class="information" style="margin-left: 30px;">
                ...
                <p><strong>자기소개:</strong>{{ user.readme }}</p>
              </div>
            </div>
            <div class="edit-button" style="margin-top: 30px; text-align: right;">
              <button @click="goToUserMypageUpdate" class="btn btn-primary">내 정보 수정하기</button>
            </div>
          </div>
          <hr>
          <!-- 구매한 비디오 리스트 뜨게 할까..? -->
    ...
    const goToUpdateReview = (review_code, ) => {
      const memberCode = sessionMember ? sessionMember.member_code : null;
      console.log('memberCode: ', memberCode);
      if (memberCode) {
        router.push({ name: 'UpdateReview', params: { member_code: memberCode, review_code: review_code } });
      } else {
        // Handle the case where member_code is not available
        console.error('Member code not available');
      }
